...
import Logo from './images/src/LOGO_new.png'


function App() {
    let [NavState, setNavState] = useState("default");
    let [clicked, setClicked] = useState("default");
    let [RenderedComponent, setRenderedComponent] = useState((<Tentang_kami_content />)); // isi ama gambar nanti
            ...
            <div className="w-5/6 m-auto">
                <div className="w-full  overflow-hidden rounded-xl m-auto flex mt-10 shadow-lg shadow-slate-800 ">
                    <div className="left-panel w-1/2 flex-1 h-auto bg-biru  p-6">
                        <div className="border-2 w-20 h-14 rounded-full text-center item-center bg-white">
                            <img src={Logo} className="w-full h-full inline-block object-fill  rounded-xl p-2" />
                        </div>
                        <div className="intro ">
                            <div className="pr-32">
                    ...
                    <div className="right-panel flex-1 h-auto">
                        <nav>
                            <ul className="flex justify-evenly items-center m-4">
                                <li className={
                                    `text-2xl p-3 rounded-full hover:bg-biru ${NavState === 'Tentang_kami_content' ? 'bg-biru' : 'bg-white'
                                    }`
                                }
                                    onClick={
                                        () => {
                                ...
                                <li className={
                                    `text-2xl p-3 rounded-full hover:bg-biru transition ${NavState === 'Kontak' ? 'bg-biru' : 'bg-white'
                                    }`
                                }
                                    onClick={
                                        () => {
                                ...
                                <li className={
                                    `hover:bg-biru text-xl p-3 rounded-full ${NavState === 'Nav_daftar_masuk' ? 'bg-biru' : 'bg-white'
                                    }`
                                }
                                    onClick={
                                        () => {
